    ...
    return (
        < div className='flex flex-col items-center  justify-center '>
           ...
           <p className='text-secondary-200 -mt-12  text-center'>We partner with the  leading brands</p> 
            ...
            {
                logo.map((el, index)=>{return <Image priority width={el.Width} height={el.Height} src={el.source} alt='logo' key={index} className='mx-3 my-2'/>
                })
            }
            </div>
            <div className='flex flex-wrap md:hidden justify-center w-full '>
                {
                    logo.map((el, index)=>{
                       return <div className='relative w-18.75 h-8.25 mx-1.5 my-2'key={index} > <Image  src={el.source} priority fill alt='logo'/></div>
